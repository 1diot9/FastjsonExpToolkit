...
def bcel_decode(encoded: str, *, decompress: bool = True) -> bytes:
    """Apache BCEL ``Utility.decode`` 的 Python 移植；接受裸编码或 ``$$BCEL$$`` 前缀。"""
    _init_char_map()
    s = encoded.strip()
    if s.startswith("$$BCEL$$"):
        s = s[len("$$BCEL$$") :]
    out = bytearray()
    i = 0
    n = len(s)
    while i < n:
        ...
        i += 1
        if i >= n:
            raise ValueError("BCEL 编码以转义符结尾")
        nxt = s[i]
        mapped = _MAP_CHAR.get(ord(nxt))
        if mapped is not None and mapped < _FREE_CHARS:
            out.append(mapped)
            i += 1
            continue
        # $HH hex
        if i + 1 >= n:
            raise ValueError("BCEL hex 转义不完整")
        hx = s[i : i + 2]
        try:
            out.append(int(hx, 16))
        except ValueError as e:
            raise ValueError(f"BCEL hex 非法: {hx!r}") from e
        i += 2
    raw = bytes(out)
    if not decompress:
        return raw
...
def class_bytes_from_bcel_code(value: str) -> bytes:
    data = bcel_decode(value)
    if not data.startswith(b"\xca\xfe\xba\xbe"):
        raise ValueError("BCEL 解码结果不是合法 .class（缺少 CAFEBABE）")
    return data
